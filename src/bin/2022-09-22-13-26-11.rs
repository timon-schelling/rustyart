...
fn update(_app: &App, model: &mut Model, _update: Update) {
    ...
    for i in 0..triangulation.triangles.len() {
        if i > triangulation.halfedges[i] || triangulation.halfedges[i] == EMPTY {
            ...
            let link = match model.links.iter().find(|link| { a == link.a && b == link.b || a == link.b && b == link.a }) {
                Some(link) => link.clone(),
                None => Link { a: a, b: b, since: SystemTime::now() },
            };
            links.push(link);
        }
    }
...
fn view(app: &App, model: &Model, frame: Frame) {
    ...
    for link in model.links.iter() {
        ...
        let color = hsla(distance_mapped / 1.5 + (app.time / 10.), 1., 0.5, distance_mapped_eased);
        draw.line()
            .color(color)
            .weight(LINE_WIGHT)
            .caps_round()
