...
trait RankeableByDistance {
    fn rank_by_distance(&self, others: &Vec<Self>) -> Vec<Self> where Self: Sized;
}
...
const PARTICLE_SPEED: f32 = 0.15;
const PARTICLE_TARGET_RADIUS: f32 = 220.;
const PARTICLE_DISTANCE_MAX: f32 = 365.;
...
fn update(_app: &App, model: &mut Model, _update: Update) {
    ...
    for particle in model.particles.iter_mut() {
        ...
        let neighbour_vec = (particle.position - neighbour.position).normalize() * (1. -neighbour_distance_mapped_eased);
