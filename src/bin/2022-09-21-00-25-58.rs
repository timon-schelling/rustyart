    ...
    for hunter in model.hunters.iter_mut() {
        if hunter.teleport {
            ...
            loop {
                ...
                let mut ranking = hunter.rank_by_distance(particles.clone());
                ranking.remove(0);
                let other = ranking.first().unwrap();
                let distance = hunter.position.distance(other.position);
                if !(distance <= other.radius + hunter.radius) {
                    hunter.neighbours = ranking.clone();
                    break;
                };
            }
            continue;
...
fn view(app: &App, model: &Model, frame: Frame) {
    ...
    for hunter in model.hunters.iter() {
        for ally in hunter.neighbours.iter().take(4) {
            draw.line()
                .color(RED)
                .weight(3.0)
                .caps_round()
    ...
    for runner in model.runners.iter() {
        for ally in runner.neighbours.iter().take(4) {
            draw.line()
                .color(BLUE)
                .weight(3.0)
                .caps_round()
